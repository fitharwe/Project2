...
function startGame() {
  ...
  player = new players(600, 450, 56, 56, "assets/curlybois2.png", true, 0);
...
function players(x, y, height, width, image, me, id) {
  this.x = x;
  this.y = y;
  this.me = me;
  this.id = id;
  ...
  this.update = function() {
    if(this.me) {
      ...
      ctx.drawImage(
        this.image,
        this.x - this.width / 2 - worldOffset.x,
        this.y - this.height / 2 - worldOffset.y,
        this.width, 
        this.height
      );
    }
...
this.socket.on('newPlayer', function (playerInfo) {
  otherPlayers.push(new players(playerInfo.x, playerInfo.y, 56, 56, "assets/curlybois2.png", false, playerInfo.playerId));
...
this.socket.on('playerMoved', function (playerInfo) {
  for(i = 0; i < otherPlayers.length; i++) {
    if(playerInfo.playerId == otherPlayers[i].id) {
      otherPlayers[i].x = playerInfo.x;
      otherPlayers[i].y = playerInfo.y;
    }
  }
});